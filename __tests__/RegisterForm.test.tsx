...
let registerUserMutationState: MutationState = {
  isLoading: false,
  isError: false,
  isSuccess: false,
}
...
describe('RegisterForm', () => {
  ...
  it('Отображение ошибки при вводе пустого поля name', async () => {
    ...
    const nameField = nameInput.closest('.form-field') as HTMLElement | null
  ...
  it('Отображение ошибки при вводе пустого поля surname', async () => {
    ...
    const surnameField = surnameInput.closest(
      '.form-field'
    ) as HTMLElement | null
  ...
  it('Отображение ошибки при вводе пустого поля password', async () => {
    ...
    if (passwordField) {
      const passwordError =
        within(passwordField).queryByText('Обязательное поле')
  ...
  it('Отображение ошибки при вводе пустого поля confirm', async () => {
    ...
    const confirmField = confirmInput.closest(
      '.form-field'
    ) as HTMLElement | null
  ...
  it('Отображение ошибки при вводе несовпадающих паролей', async () => {
    ...
    if (confirmField) {
      const confirmError = within(confirmField).queryByText(
        'Пароли не совпадают'
      )
      expect(confirmError).toBeInTheDocument()
    }
  ...
  it('Вызывает onSuccess при успешной регистрации', async () => {
    const { rerender } = render(<RegisterForm onSuccess={onSuccessMock} />)
    const {
      emailInput,
      nameInput,
      surnameInput,
    ...
    rerender(<RegisterForm onSuccess={onSuccessMock} />)
  ...
  it('Отображение ошибки при неуспешной попытке регистрации', async () => {
    ...
    const errorElement = await screen.findByText((content) =>
      content.includes('Произошла ошибка')
    )
